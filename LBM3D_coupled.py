...
if ScatterScheme == 0:
    ...
    Phasefunc = FEMphasefuncAni1  # 1 + mu*mu: Anisotropically sfx = 1 + mu
...
for cycle in range(1, 11):
    ...
    k2=1/(density*cv*cl)
    for k in range(Q):
        gcol[:, :, :, k] = f[:, :, :, k] + (geq[:, :, :, k] - f[:, :, :, k]) / tau_c - dt * F[:, :, :, k, anglei] + dt * w(k) * beta * (1-Omega) * k2 * (4*(T^4)*Stefanboltzmanzeta-G)
